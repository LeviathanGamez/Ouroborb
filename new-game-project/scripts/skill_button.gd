...
func _on_pressed() -> void:
	if GlobalGameManager.money >= price and level < max_level:
		...
		price = original_price * price_increment[current_price_increment]
		...
		tween_2.parallel().tween_property(line_2d, "default_color", Color(1.0, 1.0, 1.0, 1.0), 0.9)
...
func _on_mouse_entered() -> void:
	...
	tween.tween_property(sprite, "scale", Vector2(2.5*1.1,2.5*1.1) , 0.3)
	tween.parallel().tween_property(panel, "scale", Vector2(1.1,1.1) , 0.3)
